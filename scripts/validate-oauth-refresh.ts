 ...
 *   --no-write-back   Skip writing new tokens to storage (DANGEROUS:
 *                     invalidates stored credentials since refresh tokens
 *                     rotate)
 */
...
async function main() {
    ...
    for (const acc of accounts) {
        ...
        console.log(
            `     Access token:  ${redact(acc.credentials.accessToken)}`,
        )
        console.log(
            `     Refresh token: ${redact(acc.credentials.refreshToken)}`,
        )
        console.log(
    ...
    if (skipWriteBack) {
        ...
        if (!sameRefresh) {
            console.log(
                "   WARNING: Refresh token rotated but NOT written back.",
            )
        ...
        if (updated && updated.credentials.accessToken === data.access_token) {
            ...
            console.error(
                "   WARN: Re-read did not return updated credentials.",
            )
        }
    }
